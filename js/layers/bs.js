addLayer("bs", {
    ...
    grid: {
        ...
        getStartCost(id) {
            let cost = new Decimal(1).mul(new Decimal(10).pow(new Decimal(player.bs.total).add((id%100)+(5*(Math.floor(id/100))))))
            return cost
        },
...
player.bs.total++}
        ...
        getEffect(data, id) {
            ...
            if (player[this.layer].grid[id]>=1) eff = eff.mul(player[this.layer].grid[id]+1).pow(base).pow(id%100).pow((5*(Math.floor(id/100)-1))+1)
            return eff
        },
        getDisplay(data, id) {
