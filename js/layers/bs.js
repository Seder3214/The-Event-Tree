...
player.bs.total++}
        ...
        getEffect(data, id) {
            ...
            if (player[this.layer].grid[id]>=1) eff = eff.mul(player[this.layer].grid[id]+1).pow(base).pow(id%100+(5*((id/100)-1))).pow(player.bs.grid[id]>=10?1+(player.bs.grid[id]-9)/100:1)
            return eff
        },
        getDisplay(data, id) {
